...
class BinaryTreePrinter:
    ...
    def __treeify(self, node):
        ...
        first_line += value
        left_padding = "" if right_node_data.max_width == 0 else " " * (len(value) + 1 + self.extra_padding)
        if right_node_data.max_width > 0:
            first_line += self.branch_line * (right_node_data.root_position + self.extra_padding)
            second_line += " " * (right_node_data.root_position + len(value) + self.extra_padding) + self.right_node_line
        ...
        return NodePrintData(lines, len_before_val, len(value))
